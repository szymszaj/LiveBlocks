...
interface AnimatedCardProps {
  ...
  description: string;
  content: string;
  variant: "default" | "hover" | "pulse" | "bounce" | "rotate" | "shake" | "glow" | "slide";
  delay: number;
}

export function AnimatedCard({ title, description, content, variant, delay }: AnimatedCardProps) {
  ...
  return (
    ...
    >
      <Card className="bg-gradient-to-br from-slate-800 to-slate-900 dark:from-slate-900 dark:to-slate-950 border-slate-600 dark:border-slate-700 shadow-xl overflow-hidden">
        <CardHeader>
          <CardTitle className="text-slate-100 dark:text-slate-100">{title}</CardTitle>
          <CardDescription className="text-slate-300 dark:text-slate-400">{description}</CardDescription>
        </CardHeader>
        <CardContent>
